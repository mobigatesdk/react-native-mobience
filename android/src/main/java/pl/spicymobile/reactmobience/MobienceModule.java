package pl.spicymobile.reactmobience;
...
public class MobienceModule extends ReactContextBaseJavaModule {
    private SDK mobienceSDK;

    public MobienceModule(ReactApplicationContext reactContext) {
        super(reactContext);
    }
                ...
                while (optionsIterator.hasNext()) {
                    ...
                    switch (key) {
                        ...
                        case Constants.BUILDER_EMAIL:
                            ...
                            break;
                        case Constants.BUILDER_CUSTOM_USER_AGENT:
                            builder.setCustomUserAgent(options.getString(key));
                            break;
    ...
    public void configureDataCollectors(Boolean enable, ReadableArray _collectors) {
        try {
            ...
            if (collectors != null && collectors.length() > 0) {
                if (mobienceSDK != null)
                    mobienceSDK.configureDataCollectors(enable, Utils.JSonArray2IntArray(collectors));
            }
        } catch (JSONException ex) {
            ex.printStackTrace();
        }
    ...
    public void getEmail(Callback success, Callback error) {
        if (mobienceSDK != null)
            success.invoke(mobienceSDK.getEmail());
        else
            error.invoke("failure, init() library first!");
    }
    ...
    public void getSDKInfo(Callback success, Callback error) {
        if (mobienceSDK != null)
            success.invoke(mobienceSDK.getSDKInfo());
        else
            error.invoke("failure, init() library first!");
    }
    ...
    public void getSDKUniqueIdentifier(Callback success, Callback error) {
        if (mobienceSDK != null)
            success.invoke(mobienceSDK.getSDKUniqueIdentifier());
        else
            error.invoke("failure, init() library first!");
    }
    ...
    public void getIDsProfiles(Callback success, Callback error) {
        if (mobienceSDK != null) {
            success.invoke(Utils.intList2JSONArray(mobienceSDK.getIDsProfiles()).toString());
        } else
            error.invoke("failure, init() library first!");
    }
    ...
    public void getAdOceanTargeting(Callback success, Callback error) {
        if (mobienceSDK != null) {
            success.invoke(new JSONObject(mobienceSDK.getAdOceanTargeting()).toString());
        } else
            error.invoke("failure, init() library first!");
    }
    ...
    public void trackAppInstall(Double timestamp) {
        mobienceSDK.trackAppInstall(timestamp.longValue());
    }
